...
const BILI_FAST_CLOSE_MS = 15000;        // 连接建立后此时间内断开视为异常快速断开（轮换服务器）

// === B站 buvid Cookie 预写 ===
// 部分浏览器（如豆包）的网络环境对 B站弹幕 WS 要求握手携带 buvid Cookie（buvid3/buvid4），
// 无 Cookie 的连接握手成功后数秒即被 1006 断开（页面有 Cookie 则正常）。
// buvid 本质是客户端标识，服务端只校验格式不校验真实性，预写随机 UUID 即可与页面行为一致。
let cookiesEnsured = false;

async function ensureBilibiliCookies() {
  // chrome.cookies 仅扩展上下文可用；页面环境（协议测试）自动跳过
  if (cookiesEnsured || typeof chrome === 'undefined' || !chrome.cookies) {
    return;
  }
  try {
    const base = {
    ...
    await Promise.all([
      chrome.cookies.set({ ...base, name: 'buvid3', value: crypto.randomUUID() }),
      chrome.cookies.set({ ...base, name: 'buvid4', value: crypto.randomUUID() })
    ]);
...
class BilibiliBarrageClient {
   ...
   * @param {(roomId: string) => void} [options.onFallback] SW 直连被风控（如豆包+登录态）时的降级回调
   */
  constructor(options = {}) {
  ...
  _connect(roomId) {
    const state = {
      ...
      danmuInfo: null,   // getDanmuInfo 结果缓存 { hosts, token }，轮换 host 时复用
  ...
  /** 获取弹幕服务器地址与 token（getDanmuInfo 需要 w_rid 签名），返回全部 host 供轮换 */
  async _fetchDanmuInfo(roomId) {
    ...
    // comet 服务器（getDanmuInfo host_list）作为后备。
    ...
    console.log(`[bili] ${roomId} getDanmuInfo 成功, ${hosts.length} 个服务器: ${hosts.join(' | ')}`);
  ...
  async _openSocket(state) {
    ...
    // 部分浏览器要求握手携带 buvid Cookie，先预写（仅首次执行）
    await ensureBilibiliCookies();
    // 无缓存时获取服务器列表（轮换 host 时复用缓存,避免高频请求触发 -352 限流）
    if (!state.danmuInfo) {
      try {
        ...
        this._scheduleRetry(state, delay);
        return;
      }
    }
    ...
    const ws = state.hostIndex === 0
      ? new WebSocket(url, 'proto') // broadcast 新通道要求 proto 子协议（hosts[0] 固定为 broadcast）
      : new WebSocket(url);
    ...
    ws.onopen = () => {
      console.log(`[bili] ${state.roomId} WS 已连接 (real=${state.danmuRoomId})`);
    ...
    ws.onclose = (e) => {
      ...
      console.log(`[bili] ${state.roomId} WS 关闭 code=${e.code} fast=${fastClose} stopped=${state.stopped}`);
      if (state.stopped) {
        return;
      }
      if (fastClose) {
        ...
        if (state.hostIndex >= state.danmuInfo.hosts.length) {
          ...
          state.hostIndex = 0;
          state.fullCycleFails = (state.fullCycleFails || 0) + 1;
          console.warn(`[bili] ${state.roomId} 所有服务器均异常断开,${BILI_RATE_LIMIT_DELAY}ms 后重新获取服务器列表`);
          // 连续多轮全部断开 → SW 直连通道被风控（如豆包+登录态），通知上层降级到页面通道
          if (state.fullCycleFails >= 2 && this.onFallback) {
            console.warn(`[bili] ${state.roomId} SW 直连连续异常,触发页面通道降级`);
            this.onFallback(state.roomId);
          }
          this._scheduleRetry(state, BILI_RATE_LIMIT_DELAY);
          return;
        }
        console.warn(`[bili] ${state.roomId} 1006 快速断开,轮换服务器 ${state.hostIndex + 1}/${state.danmuInfo.hosts.length}`);
        this._scheduleRetry(state, 1000); // 换服务器用短退避
        return;
      }
      // 正常断开,指数退避重连
      this._scheduleRetry(state, state.retryDelay);
    };
  ...
  /** 安排重连（指数退避，可指定本次延迟） */
  _scheduleRetry(state, delay = state.retryDelay) {
    clearTimeout(state.retryTimer);
