...
//   sync({danmakuWatch, viewerEnabled, hasBiliRoom})
//                     通道决策：检测需求 / 该平台观众数开关 / 有无 B站房间 / 登录态 / 持久化降级标记
//                     → 产出通道状态。事实由编排从房间库读出后传入（本 module 不读 rooms/streamers/settings）
...
class BiliBridgeChannel {
  ...
  /**
   * 同步通道状态：按传入的三个事实（检测需求 / 该平台观众数开关 / 有无 B站房间）与登录态、
   * 持久化降级标记决策通道，并处理副作用（关桥接页 / 持久化 / 切换日志）。
   * @param {object} [facts]
   * @param {boolean} [facts.danmakuWatch=false] 是否存在启用了弹幕检测的 B站房间（检测需求）
   * @param {boolean} [facts.viewerEnabled=true] 该平台观众数开关是否开启
   * @param {boolean} [facts.hasBiliRoom=true] 房间列表里是否有 B站房间
   */
  async sync({ danmakuWatch = false, viewerEnabled = true, hasBiliRoom = true } = {}) {
    // 弹幕需求 = 观众数采样 或 弹幕检测（两者正交，任一存在即需要 B站弹幕通道）
    if (!viewerEnabled && !danmakuWatch) {
      this._enabled = false;
