...
//   sync({watchNeed, viewerEnabled, hasBiliRoom})
//                     通道决策：盯守需求（弹幕检测或弹幕激增，任一存在即可）/ 该平台观众数开关 /
//                     有无 B站房间 / 登录态 / 持久化降级标记 → 产出通道状态。
//                     事实由编排从房间库读出后传入（本 module 不读 rooms/streamers/settings）
...
//   enableFallback({viewerEnabled})
//                     SW 直连采样被风控 → 置内存标记 + 持久化 + 日志（停客户端与发通知由编排完成）
//   enabled getter    编排读此结果做采样/检测分支（页面桥接 vs SW 直连）
//
...
class BiliBridgeChannel {
  ...
  /**
   * 同步通道状态：按传入的三个事实（盯守需求 / 该平台观众数开关 / 有无 B站房间）与登录态、
   * 持久化降级标记决策通道，并处理副作用（关桥接页 / 持久化 / 切换日志）。
   * @param {object} [facts]
   * @param {boolean} [facts.watchNeed=false] 是否存在有盯守需求的 B站房间（弹幕检测或弹幕激增任一）
   * @param {boolean} [facts.viewerEnabled=true] 该平台观众数开关是否开启
   * @param {boolean} [facts.hasBiliRoom=true] 房间列表里是否有 B站房间
   */
  async sync({ watchNeed = false, viewerEnabled = true, hasBiliRoom = true } = {}) {
    // 弹幕需求 = 观众数采样 或 盯守（两种盯守需求都算，两者正交，任一存在即需要 B站弹幕通道）
    if (!viewerEnabled && !watchNeed) {
      this._enabled = false;
   ...
   * 属编排（onFallback 回调），不在此处理。
   * @param {object} [facts]
   * @param {boolean} [facts.viewerEnabled=true] 该平台观众数开关是否开启（关闭时不降级）
   */
  async enableFallback({ viewerEnabled = true } = {}) {
    if (this._enabled || !viewerEnabled) {
      return;
    }
    this._enabled = true;
