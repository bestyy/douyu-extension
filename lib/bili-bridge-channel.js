...
//   sync()            通道决策：观众数开关 / 检测需求 / 房间列表有无 B站 / 登录态 / 持久化降级标记 → 产出通道状态
//   sample(roomIds)   页面通道采样：打开（或复用）桥接标签页并驱动一轮采样
//   watch(roomIds)    页面通道检测：把盯守房间列表下发给桥接页（弹幕检测长连接常驻）
//   close()           关闭桥接标签页（采样完成/超时、关开关、无 B站房间、盯守结束时调用）
//   enableFallback()  SW 直连采样被风控 → 置内存标记 + 持久化 + 日志（编排由 background 完成）
//   enabled getter    background 读此结果做采样/检测分支（页面桥接 vs SW 直连）
//
...
class BiliBridgeChannel {
  ...
  /**
   * 同步通道状态：查询「观众数开关 / 检测需求 / 房间列表有无 B站 / 登录态 / 持久化降级标记」，
   * 产出通道状态（enabled）并处理副作用（清 B站高能榜数 / 关桥接页 / 持久化 / 切换日志）。
   * 通道决策：
   *   - 既不需要观众数也不需要检测 → 停用通道 + 关桥接页
   *   - 关闭 B站观众数开关 → 清 streamers 的 rankCount（斗鱼 vipCount 不动）；
   *     若仍有检测需求则通道继续按下述规则决策（检测与观众数开关正交）
   *   - 无 B站房间 → 停用通道 + 关桥接页（但不清 streamers：斗鱼贵宾数仍有效）
   *   - 有 B站房间且需要弹幕 → 登录态（SESSDATA）下 SW 直连握手必被 1006 风控，必须走页面桥接；
   *     未登录时 SW 直连可用，但持久化的降级标记（未登录时被风控降级过）在 SW 重启后
   *     继续生效，与登录检测共同决定通道。
   * @param {object} [options]
   * @param {boolean} [options.danmakuWatch=false] 是否存在启用了弹幕检测的 B站房间（检测需求）
   */
  async sync({ danmakuWatch = false } = {}) {
    const settings = await this._storage.get('settings');
    // B站观众数开关独立控制（fetchBilibiliViewerCount）；新字段未写入时回退旧总开关 fetchViewerCount
    const biliViewerEnabled = this._isBiliViewerEnabled(settings);
    const rooms = (await this._storage.get('rooms')) || [];
    const hasBili = rooms.some(r => r.platform === 'bilibili');

    if (!biliViewerEnabled) {
      // 关闭 B站观众数获取：清除已存高能榜在线数（斗鱼贵宾数 vipCount 不在此清理，
      // 由 background 按斗鱼开关处理）。检测仍可能需要通道，故不在此停用。
      await this._stripRankCounts();
    }
    // 弹幕需求 = 观众数采样 或 弹幕检测（两者正交，任一存在即需要 B站弹幕通道）
    if (!biliViewerEnabled && !danmakuWatch) {
      this._enabled = false;
  ...
  /**
   * 页面通道检测：把盯守房间列表下发给桥接页，页面侧保持检测长连接（弹幕检测）。
   * 与 sample 的区别：不等待完成信号、不关闭页面——盯守期间桥接页常驻，
   * 由 background 在盯守结束时 close()（或每轮轮询按最新盯守列表重新下发，幂等收敛）。
   * 空列表只用于清掉页面上的残留检测连接（SW 重启后内存态丢失时收敛），不会为此开页。
   */
  async watch(roomIds) {
    const ids = [...new Set((roomIds || []).map(String))];
    const tabId = ids.length > 0 ? await this._ensureTab() : await this._findTab();
    if (tabId === null) {
      return; // 没有盯守房间且无桥接页：无需下发
    }
    if (ids.length === 0) {
      try {
        await this._tabs.sendMessage(tabId, { type: 'BILI_WATCH_ROOMS', roomIds: [] });
      } catch (e) {
        // 页面已不可用：其连接随页面一起失效，无需处理
      }
      return;
    }
    const ready = await this._waitReady(tabId);
    if (!ready) {
      console.warn('[bili] 桥接页长时间未就绪, 本轮检测下发跳过');
      return;
    }
    try {
      await this._tabs.sendMessage(tabId, { type: 'BILI_WATCH_ROOMS', roomIds: ids });
    } catch (e) {
      console.warn('[bili] 驱动桥接页检测失败, 本轮跳过');
    }
  }
  ...
  async enableFallback() {
    const settings = await this._storage.get('settings');
    if (this._enabled || !this._isBiliViewerEnabled(settings)) {
      return;
    }
    this._enabled = true;
  ...
  // B站高能榜开关是否开启：新字段 fetchBilibiliViewerCount 优先；未写入时回退旧总开关
  // fetchViewerCount 语义（字段不存在视为开启）
  _isBiliViewerEnabled(settings) {
    if (settings?.fetchBilibiliViewerCount !== undefined) {
      return settings.fetchBilibiliViewerCount !== false;
    }
    return settings?.fetchViewerCount !== false;
  }
  // 清除 streamers 中的 B站高能榜字段（关闭 B站观众数开关时调用，避免 popup 显示过期数据）
  async _stripRankCounts() {
    const streamers = (await this._storage.get('streamers')) || [];
    if (!streamers.some(s => 'rankCount' in s)) {
      return;
    }
    const cleaned = streamers.map(s => {
      const { rankCount, ...rest } = s;
      return rest;
    });
    await this._storage.set('streamers', cleaned);
  }

  // 查找现有桥接标签页（不创建、不刷新），无则返回 null
  async _findTab() {
    const tabs = await this._tabs.query({ url: 'https://live.bilibili.com/*' });
