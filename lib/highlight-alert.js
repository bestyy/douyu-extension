...
//
// 平台门 HIGHLIGHT_PLATFORMS 留在本模块（常量归属判据见 ADR-0005）：它是这个判定的口径，
// 只有斗鱼有这个形态的信息，跟着判定走才能在单测里单独 require 驱动。本模块内部没有函数读它——
// 读它的是编排取数入口的三层门控（平台门 → 总开关 → 该房开关）与设置页的房间面板。
// 取数与字段收敛在平台 API 层，水位落盘与通知发送在编排。
//
