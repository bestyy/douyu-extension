 ...
 * @param {boolean} [options.enabled] 总开关（关闭时返回空计划，见 isDanmakuWatchEnabled）
 * @returns {{active: Array<{key, platform, roomId, nickname, config}>, queued: Array<{key, platform, roomId, nickname}>}}
 */
function selectWatchPlan(rooms, onlineKeys, { limit = WATCH_MAX_CONCURRENT, enabled = true } = {}) {
  const active = [];
  const queued = [];
  if (!enabled) {
    return { active, queued }; // 总开关关闭：既不盯守也不排队（排队提示随之消失）
  }
  for (const room of rooms || []) {
    if (!WATCH_PLATFORMS.includes(room.platform)) {
      continue; // 无弹幕通道的平台不支持检测
    }
 ...
 * @param {boolean} [enabled] 总开关（关闭时不备通道）
 * @returns {boolean}
 */
function hasConfiguredBiliWatch(rooms, enabled = true) {
  return enabled && (rooms || []).some(r => r.platform === 'bilibili' && normalizeWatch(r.watch));
}
