...
//
// 盯守计划同时服务弹幕检测与弹幕激增（见 ADR-0004）：两种需求各自的门控在 selectWatchPlan 里合成
// 同一份计划，共用长连接与名额池；激增的计数与判定在 lib/danmaku-surge.js。
//
...
const WATCH_PLATFORMS = ['douyu', 'bilibili']; // 具备弹幕通道的平台，其余不参与检测
...
/**
 * 弹幕检测总开关（settings.danmakuWatchEnabled，默认开启；旧版本无该字段视为开启）。
 * 关闭时全局停止盯守：不建长连接、不排队、不触发检测通知（各房已配的检测词保留）。
 * @param {{danmakuWatchEnabled?: boolean}} [settings] 存储中的 settings
 * @returns {boolean}
 */
function isDanmakuWatchEnabled(settings) {
  return settings?.danmakuWatchEnabled !== false;
}

/**
 * 盯守计划：按房间列表顺序取前 limit 个「有盯守需求且开播」的房间为盯守对象，其余在线房间排队。
 * 盯守需求有两种，各自受自己的总开关约束，互不牵连（见 ADR-0004）：
 * - 检测需求：检测总开关开启 + 该房配了检测词（归一化后的检测配置随条目带出）
 * - 激增需求：激增总开关开启 + 该房打开了激增提醒（条目标记 surge，检测配置可为 null）
 * 两种需求共用同一个名额池（5 个），不新增连接，优先级只看房间列表顺序（用户用拖拽排序表达）。
 * 纯函数：每轮轮询用最新在线快照重算即可，下播释放名额后由列表顺序靠前的排队房间补位（不抢占在线房间）。
 * @param {Array<{roomId: string, platform: string, nickname?: string, watch?: object, surgeAlert?: boolean}>} rooms
 *        存储中的 rooms 数组（顺序即优先级）
 * @param {Set<string>} onlineKeys 在线房间复合键集合（`platform_roomId`）
 * @param {object} [options]
 * @param {number} [options.limit] 同时盯守上限
 * @param {boolean} [options.watchEnabled] 检测总开关（关闭时该房不因检测占用名额，检测配置不参与）
 * @param {boolean} [options.surgeEnabled] 激增总开关（关闭时该房不因激增占用名额）
 * @returns {{active: Array<{key, platform, roomId, nickname, config, surge}>, queued: Array<{key, platform, roomId, nickname}>}}
 */
function selectWatchPlan(rooms, onlineKeys, { limit = WATCH_MAX_CONCURRENT, watchEnabled = true, surgeEnabled = true } = {}) {
  const active = [];
  const queued = [];
  for (const room of rooms || []) {
    if (!WATCH_PLATFORMS.includes(room.platform)) {
      continue; // 无弹幕通道的平台不参与盯守
    }
    const config = watchEnabled ? normalizeWatch(room.watch) : null;
    const surge = surgeEnabled && room.surgeAlert === true;
    if (!config && !surge) {
      continue; // 两种盯守需求都没有：不盯也不排队（排队提示随之消失）
    }
...
/**
 * 是否存在有盯守需求的 B站房间（B站通道决策的输入：只要有需求就要备好通道，
 * 与该房当前是否开播无关——通道要在开播前就位）。检测与激增任一存在即算需求，
 * 否则只开激增的 B站房间不会备好通道（见 ADR-0004）。
 * @param {Array<{roomId: string, platform: string, watch?: object, surgeAlert?: boolean}>} rooms
 * @param {object} [gates]
 * @param {boolean} [gates.watchEnabled] 检测总开关（关闭时不算检测需求）
 * @param {boolean} [gates.surgeEnabled] 激增总开关（关闭时不算激增需求）
 * @returns {boolean}
 */
function hasBiliWatchNeed(rooms, { watchEnabled = true, surgeEnabled = true } = {}) {
  return (rooms || []).some(room => {
    if (room.platform !== 'bilibili') {
      return false;
    }
    if (watchEnabled && normalizeWatch(room.watch)) {
      return true;
    }
    return surgeEnabled && room.surgeAlert === true;
  });
}
