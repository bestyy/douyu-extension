// lib/orchestrator.js — 直播状态编排：轮询 / 观众数采样 / 弹幕盯守 / 看点轮询 / 今日统计取数 / 五类通知
//
...
//             highlightWatermarks 看点水位 / todayStats 今日统计 / popupCategoryId 弹窗分类栏选中项）
//   apis      { douyu: { batchFetchRoomInfo, resolveNickname, fetchHighlights }, bilibili: {...} } 平台 API
//   todayStatsApi 今日统计取数 port（lib/doseeing-api.js）：一个模块对一个接口提供方，故不进 apis（见 ADR-0007）
...
//             lib/highlight-alert.js + lib/today-stats.js 的导出
...
// 与今日统计的平台门/保鲜/显示判定留在五个纯规则 module；本 module 只负责「什么时候问、拿到结果做什么」。
// 通知 ID 与直播间 URL 的对应也在本 module（见 CONTEXT.md「通知」）。
...
const ORCHESTRATOR_TODAY_STATS_ALARM = 'todayStatsPoll'; // 今日统计取数的节拍（5 分钟一轮）
...
const ORCHESTRATOR_TODAY_STATS_INTERVAL = 5; // 今日统计取数间隔（分钟）：当日累计变化慢，5 分钟够用，见 ADR-0007
...
const ORCHESTRATOR_HIGHLIGHT_SUFFIX = '_highlight';   // 看点通知 ID 后缀（同上）
...
const ORCHESTRATOR_NOTIFICATION_SUFFIXES = [
  ...
  ORCHESTRATOR_HIGHLIGHT_SUFFIX
];

function createOrchestrator({ store, keyValue, apis, todayStatsApi, clients, bridge, notifier, rules, identity, alarms, tabs }) {
  ...
  let highlightAlarmOn = false;     // 看点取数 alarm 是否已创建（同上）
  let todayStatsAlarmOn = false;    // 今日统计取数 alarm 是否已创建（同上）
  ...
  /**
   * 房间通知的统一创建入口：开播 / 弹幕命中 / 观众数提醒 / 弹幕激增 / 看点五种通知外形一致，
  ...
  function notificationTitlePrefix(platform) {
    return `[${identity.platformLabel(platform)}]`;
  }

  /** 通知 ID 与直播间的对应（开播通知为 `platform_roomId`，另四种带后缀；房间号恒为纯数字） */
  function liveUrlFromNotificationId(notificationId) {
    ...
    const ref = identity.roomRefFromKey(baseId);
    return ref ? identity.liveUrl(ref) : null;
  }
  ...
  async function notifyViewerAlert(platform, { roomId, nickname }, value, threshold, streamer) {
    const meta = identity.viewerMetric(platform);
    await createRoomNotification(`${identity.roomKey({ platform, roomId })}${ORCHESTRATOR_VIEWER_ALERT_SUFFIX}`, {
      title: `${notificationTitlePrefix(platform)} ${nickname} ${meta.label}超过 ${threshold}！`,
   ...
   *
   * 条数与样本弹幕都放正文而不是上下文行：Windows 上 contextMessage 被渲染成通知底部「应用名 + 时间」
   ...
   * 条数与样本弹幕就是这条通知的全部内容，不能赌平台渲染；房间标题是次要信息，留在上下文行。
   *
   * 样本弹幕（裁决那一分钟里最有内容的一条）是「现在在聊什么」的唯一线索，放在条数下面一行；
   * 那一分钟里全是复读刷屏时没有合格样本，就只报条数（见 danmaku-surge 的评选口径）。
   */
  async function notifySurgeAlert(entry, { bucketCount, baseline, sample }) {
    ...
    const message = `上一分钟 ${formatNumber(bucketCount)} 条，平时约 ${formatNumber(Math.round(baseline))} 条`
      + (sample ? `\n「${truncateDanmu(sample)}」` : '');
    await createRoomNotification(`${entry.key}${ORCHESTRATOR_SURGE_SUFFIX}`, {
      title: `${notificationTitlePrefix(entry.platform)} ${entry.nickname} 弹幕激增！`,
      message,
      contextMessage: (streamer && streamer.title) || '正在直播'
    });
  }
  ...
  async function notifyDanmakuHit(entry, { keyword, text, user, count }) {
    ...
    await createRoomNotification(`${entry.key}${ORCHESTRATOR_WATCH_SUFFIX}`, {
      title: `${notificationTitlePrefix(entry.platform)} ${entry.nickname} 弹幕命中！`,
      message: body,
   ...
   * 每轮重建会把取数一直往后推（照 danmakuSurgeTick 的收敛法）。
   */
  async function syncHighlightAlert() {
    ...
    if (wanted && !highlightAlarmOn) {
      alarms.create(ORCHESTRATOR_HIGHLIGHT_ALARM, { periodInMinutes: ORCHESTRATOR_HIGHLIGHT_INTERVAL });
      highlightAlarmOn = true;
    } else if (!wanted && highlightAlarmOn) {
      alarms.clear(ORCHESTRATOR_HIGHLIGHT_ALARM);
      highlightAlarmOn = false;
    }
  }
  ...
  async function pollHighlights() {
    ...
    const fetched = await Promise.all(targets.map(async room => ({
      room,
      result: await apis[room.platform].fetchHighlights(room.roomId)
    })));
  ...
  /**
   * 看点通知：ID 为派生 ID（房间复合键 + _highlight 后缀），同一房间反复触发复用同一 ID（覆盖不堆积）。
   * 正文放看点标题（一次发现多条时补一行「另有 N 条」——逐条发只会多响几声，用户看到的还是最后那条），
   * 房间标题放上下文行（Windows 上上下文行不渲染，因此标题必须进正文）。
  ...
  async function notifyHighlight(room, highlight, count) {
    ...
    const title = highlight.title || '新看点';
    await createRoomNotification(`${identity.roomKey(room)}${ORCHESTRATOR_HIGHLIGHT_SUFFIX}`, {
      title: `${notificationTitlePrefix(room.platform)} ${room.nickname} 有新看点！`,
      message: count > 1 ? `${title}\n另有 ${count - 1} 条` : title,
      contextMessage: (streamer && streamer.title) || '正在直播'
    });
  }
  ...
  // === 今日统计（每 5 分钟取一次第三方数据站，只读展示不通知，见 ADR-0007）===
  // 平台门 / 在线门 / 保鲜 / 显示判定全在 lib/today-stats.js；本 module 只管取数节拍、
  // 结果落盘与清理。结果是编排自己的键（`{ "douyu_9999": { chatPv, …, date, fetchedAt } }`，
  // 照 highlightWatermarks），不进房间库——它不是「开播信息快照」（ADR-0003 的单写者边界不动）。

  /** 该房是否属于今日统计的平台门（是否开播由调用方各自判断） */
  function isTodayStatsRoom(room) {
    return rules.STATS_PLATFORMS.includes(room.platform);
  }

  /**
   * 今日统计取数 alarm 的收敛点（房间增删、设置变更与每轮轮询都经过）：只在「总开关开启 且
   * 至少一个斗鱼房间」时存在，全关掉即清除（不给 SW 留无用唤醒）；已在跑不重建——create 会重置计时，
   * 每轮重建会把取数一直往后推（照 syncHighlightAlert 的收敛法）。
   */
  async function syncTodayStats() {
    const snapshot = await store.snapshot();
    const wanted = rules.isTodayStatsEnabled(snapshot.settings) && snapshot.rooms.some(isTodayStatsRoom);
    if (wanted && !todayStatsAlarmOn) {
      alarms.create(ORCHESTRATOR_TODAY_STATS_ALARM, { periodInMinutes: ORCHESTRATOR_TODAY_STATS_INTERVAL });
      todayStatsAlarmOn = true;
    } else if (!wanted && todayStatsAlarmOn) {
      alarms.clear(ORCHESTRATOR_TODAY_STATS_ALARM);
      todayStatsAlarmOn = false;
    }
  }

  /**
   * 取数与落盘：只取「总开关开启 + 当前开播」的斗鱼房间（下播房间的卡片本就不显示，不产生没用处的
   * 请求），并行取数后逐房处理——失败只记日志、**不动该房旧值**（一次接口抖动不该让数字忽隐忽现）；
   * 成功且值有变化才写回，`date` 用 statsDateKey 盖本地当天戳（与弹窗判废同一个口径）。
   */
  async function pollTodayStats() {
    const snapshot = await store.snapshot();
    if (!rules.isTodayStatsEnabled(snapshot.settings)) {
      return; // 总开关关闭：不请求该站（已落盘的数字保留，重新打开即恢复）
    }
    const targets = rules.selectTodayStatsTargets(snapshot.rooms);
    if (targets.length === 0) {
      return;
    }

    const records = (await keyValue.get('todayStats')) || {};
    const today = rules.statsDateKey(Date.now());
    // 并行取数（fetchTodayStats 自带 try/catch，不会抛）：SW 生命周期有限，
    // 逐房串行会把每房的 10 秒超时叠起来
    const fetched = await Promise.all(targets.map(async room => ({
      room,
      result: await todayStatsApi.fetchTodayStats(room.roomId)
    })));
    ...
    for (const { room, result } of fetched) {
      ...
      if (!result || result.success !== true || !result.data) {
        console.warn(`今日统计取数失败: ${key}`, (result && result.error) || 'unknown');
        continue;
      }
      const { chatPv, chatUv, giftAmount, giftUv, ts } = result.data;
      const next = { chatPv, chatUv, giftAmount, giftUv, date: today, fetchedAt: ts };
      const prev = records[key];
      if (prev && prev.chatPv === next.chatPv && prev.chatUv === next.chatUv
        && prev.giftAmount === next.giftAmount && prev.giftUv === next.giftUv && prev.date === next.date) {
        continue; // 数字与日期都没变：不写回（避免每 5 分钟白写一次存储）
      }
      records[key] = next;
      changed = true;
    }
    if (changed) {
      await keyValue.set('todayStats', records);
    }
  }

  /** 清掉某房今日统计（房间移除）：与通知水位、notifiedRooms 的处理挨在同一处 */
  async function clearTodayStats(ref) {
    const records = (await keyValue.get('todayStats')) || {};
    const key = identity.roomKey(ref);
    if (!(key in records)) {
      return; // 该房没有记录：不必白写一次存储
    }
    delete records[key];
    await keyValue.set('todayStats', records);
  }
  ...
  async function checkNewLiveStreams({ wentLive, streamers }) {
    ...
    for (const streamer of wentLive) {
      ...
      // 统计文案按平台区分：数值字段与指标名都取自房间标识 module（斗鱼贵宾 / B站高能榜）
      const metric = identity.viewerMetric(streamer.platform);
      const field = identity.viewerField(streamer.platform);
      const count = field ? streamer[field] : undefined;
      ...
      const created = await createRoomNotification(compositeKey, {
        title: `${notificationTitlePrefix(streamer.platform)} ${streamer.nickname} 开播了！`,
  ...
  /** SW 唤醒时重建内存态：观众数/B站通道状态 + 检测盯守配置 + 看点/今日统计取数收敛（检测配置与计数都在内存里） */
  function start() {
    watchReady = (async () => {
      await syncViewerSettings();
      await syncDanmakuWatch();
  ...
  async function onMessage(message) {
    switch (message && message.type) {
      ...
      case 'PATCH_SETTINGS': {
        ...
        await syncViewerSettings();
        await syncDanmakuWatch();
        await syncHighlightAlert(); // 看点总开关可能刚被改过（取数 alarm 随之收敛）
        await syncTodayStats(); // 今日统计总开关同理
  ...
  async function handleRemoveRoom(roomId, platform) {
    ...
    // 今日统计同理：移除后不再为它保留这份数据
    await clearTodayStats({ platform, roomId });
    await syncHighlightAlert();
    await syncTodayStats();
  ...
  async function onAlarm(name) {
    ...
    } else if (name === ORCHESTRATOR_TODAY_STATS_ALARM) {
      await pollTodayStats();
    }
  }
