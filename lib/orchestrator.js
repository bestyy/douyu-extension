// lib/orchestrator.js — 直播状态编排：轮询 / 观众数采样 / 弹幕盯守 / 看点轮询 / 五类通知
//
...
//   rules     纯规则：lib/viewer-alert.js + lib/danmaku-watch.js + lib/danmaku-surge.js + lib/highlight-alert.js 的导出
...
// 阈值判定（观众数提醒）、命中判定（弹幕检测）、激增裁决（弹幕激增）与「什么算新看点」（看点通知）
// 留在四个纯规则 module；本 module 只负责「什么时候问、拿到结果做什么」。通知 ID 与直播间 URL 的对应
// 也在本 module（见 CONTEXT.md「通知」）。
...
function createOrchestrator({ store, keyValue, apis, clients, bridge, notifier, rules, identity, alarms, tabs }) {
  ...
  async function evaluateViewerAlert(platform, result) {
    if (!identity.viewerMetric(platform)) {
      return false;
    }
  ...
  async function syncHighlightAlert() {
    const snapshot = await store.snapshot();
    const wanted = rules.isHighlightAlertEnabled(snapshot.settings) &&
      snapshot.rooms.some(room => rules.HIGHLIGHT_PLATFORMS.includes(room.platform) && room.highlightAlert === true);
  ...
  async function pollHighlights() {
    ...
    const targets = snapshot.rooms.filter(room =>
      rules.HIGHLIGHT_PLATFORMS.includes(room.platform) &&
      room.highlightAlert === true &&
      room.online === true
    );
    if (targets.length === 0) {
      return;
  ...
  /** SW 唤醒时重建内存态：观众数/B站通道状态 + 检测盯守配置 + 看点取数收敛（检测配置与计数都在内存里） */
  function start() {
    watchReady = (async () => {
      await syncViewerSettings();
      await syncDanmakuWatch();
