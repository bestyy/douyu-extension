...
//
// 采样超时的兜底要问一次「该房是否开播」：这个平台事实的唯一实现在 lib/douyu-api.js
// （见 CONTEXT.md「开播状态」），本模块经构造注入的 probeOnline port 取用，不在此重复判定。
// 拿到结论后「按 0 上报 / 重连一次 / 跳过 / 放弃」的裁决由零依赖纯函数 decideSampleTimeout
// 承担，因此这条分支可脱离 WebSocket 与定时器单独驱动（本模块 UMD 导出，node 下可 require）。
//
// 检测模式（长连接，见 ADR-0001）：斗鱼在现有 oni（贵宾数）分支旁解析 chatmsg（弹幕文本
// txt / 发送者 nn），经 onDanmu 回调扇入 background 的弹幕检测。长连接由开播门控驱动
// （setRooms() 按盯守房间列表增删连接），keepalive 心跳保活 + 断开指数退避重连。
// 采样短连与检测长连接为两个独立实例（连接互不干扰，见 ADR-0001）。
...
const SAMPLE_TIMEOUT_MS = 20000; // 采样超时：到时问一次开播状态，裁决见 decideSampleTimeout
...
/**
 * 采样超时的裁决：这次超时该做什么。纯函数——不碰 IO、不碰定时器，可脱离 WebSocket 单独驱动。
 *
 * 开播状态由调用方问一次（probeOnline port），三态各有去处：
 * - `online === false`：未开播，没有 oni 推送，按贵宾数 0 上报（斗鱼页面未开播同样显示 0，
 *   不显示会让用户误以为漏采）
 * - `online === true` 且未重试过：在播却没收到的可能只是网络抖动，立刻重连重试一次
 * - `online === true` 且已重试过：本轮跳过，等下一轮采样
 * - `online === undefined`（查不到）：本轮放弃。这是一个明确的结论，不是「什么都别做」的沉默
 *
 * `reported` 优先于 `online`：探测最长可能花 10 秒（fetchRoomInfo 的超时），期间 oni 可能
 * 已经到达并上报，此时不能再用一个陈旧的探测结果把已上报的值覆盖成 0。
 *
 * @param {object} facts
 * @param {boolean|undefined} [facts.online] 开播状态（undefined = 查不到）
 * @param {boolean} [facts.retried] 这是超时后的重试连接
 * @param {boolean} [facts.reported] 本轮该房已上报过贵宾数
 * @returns {'ignore'|'report-zero'|'retry'|'skip'|'abandon'}
 */
function decideSampleTimeout({ online, retried = false, reported = false } = {}) {
  if (reported) {
    return 'ignore';
  }
  if (online === false) {
...
class BarrageClient {
   ...
   * @param {(roomId: string) => Promise<boolean|undefined>} options.probeOnline 开播状态查询
   *        （采样超时兜底用；生产 adapter 见 background.js，指向斗鱼平台 API）
   ...
   * @param {{setTimeout?: Function, clearTimeout?: Function}} [options.timers] 定时器（缺省取全局；测试注入可控实现）
   */
  constructor({ probeOnline, onVipCount, onDanmu, timers } = {}) {
    if (typeof probeOnline !== 'function') {
      throw new TypeError('BarrageClient 需要 probeOnline port：(roomId) => Promise<boolean|undefined>');
    }
    this.probeOnline = probeOnline;
    this.onVipCount = onVipCount || null;
    this.onDanmu = onDanmu || null;
    // 定时器先取裸函数、再包一层调用：必须以普通函数形式落地。把全局 setTimeout 直接存成
    // 属性、然后 this._timers.setTimeout(...) 调用时，this 是这个对象而不是全局，
    // Chrome 的 WebIDL 会抛 Illegal invocation（node 不检查接收者，所以只有浏览器里现形）。
    const setTimer = (timers && timers.setTimeout) || setTimeout;
    const clearTimer = (timers && timers.clearTimeout) || clearTimeout;
    this._timers = {
  ...
  _connect(roomId, retried = false) {
    const state = {
      ...
      timeoutTimer: this._timers.setTimeout(() => this._handleTimeout(state), SAMPLE_TIMEOUT_MS),
      stopped: false
    };
  ...
  /**
   * 采样超时收尾：问一次开播状态（probeOnline port），把结论交给纯判定决定做什么（见 decideSampleTimeout）。
   * 未开播 → 按贵宾数 0 上报；在播但没收到的可能只是抖动 → 立刻重连重试一次；查不到 → 本轮放弃。
   */
  async _handleTimeout(state) {
    if (state.reported || state.stopped) {
      return; // 已经收尾过（或连接已停）：不必白问一次开播状态
    }
    this._disconnect(state.roomId);
    // 探测最长约 10 秒，期间 oni 可能已到达并上报（state.reported 变真），因此探测之后再裁决，
    // 让「已上报」归到 'ignore'、不被一个陈旧的探测结果覆盖成 0
    const online = await this._probeOnline(state.roomId);
    const action = decideSampleTimeout({
      online,
      retried: state.retried,
      reported: state.reported
    });

    if (action === 'report-zero') {
      console.log(`[dy] ${state.roomId} 未开播, 按贵宾数 0 上报`);
      state.reported = true;
      if (this.onVipCount) {
    ...
    if (action === 'retry') {
      // 开播中但超时未收到 oni → 立即重试一次（重试连接有独立超时，仍超时则走 'skip'）
    ...
    if (action === 'ignore') {
      // 探测期间 oni 已到达并上报（state.reported 变真）：这次超时不用再做任何事，也不是「查不到」
      console.log(`[dy] ${state.roomId} 探测期间已收到 oni, 超时收尾忽略`);
      return;
    }
    console.warn(`[dy] ${state.roomId} 采样超时且查不到开播状态, 本轮放弃`);
  }

  /**
   * 查一次开播状态：port 的返回收敛成三态布尔（只认 true / false，其余与异常都算「查不到」）。
   * 查不到是一个明确的结论（纯判定的 'abandon'），不是静默 return。
   */
  async _probeOnline(roomId) {
    try {
      const online = await this.probeOnline(roomId);
      return online === true || online === false ? online : undefined;
    } catch (e) {
      console.warn(`[dy] ${roomId} 查询开播状态失败: ${e.message}`);
      return undefined;
    }
  }
  ...
  _disconnect(roomId) {
    ...
    state.stopped = true;
    this._timers.clearTimeout(state.timeoutTimer);
    this._timers.clearTimeout(state.retryTimer);
  ...
  _scheduleRetryLong(state) {
    ...
    state.retryTimer = this._timers.setTimeout(() => {
      if (!state.stopped) {
        this._openLongSocket(state);
      }
    }, state.retryDelay);
...
// （协议编解码与采样超时裁决都是零依赖纯函数，导出后可在单测里单独驱动）
