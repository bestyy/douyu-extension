...
const BilibiliAPI = {
  ...
  async fetchRoomInfo(roomId) {
    try {
      ...
      if (uid) {
        try {
          // 使用 Bilibili 公开 card 接口获取用户昵称和头像
          ...
          if (userResp.ok) {
            ...
            if (userData.code === 0 && userData.data?.card) {
              nickname = userData.data.card.name || '';
              avatarUrl = userData.data.card.face || '';
            }
          }
        } catch (e) {
      ...
      // 兜底：从 room info 拿主播名（部分房间信息中可能包含 uname）
      if (!nickname) {
        nickname = d.uname || '';
      }
